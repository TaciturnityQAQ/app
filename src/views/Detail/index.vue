...
export default {
  ...
  methods: {
    ...
    //表单元素修改产品个数
    changeSkuNum(event){
      //用户输入进来的文本*1 可以判断用户输入的是否是数字
      let value = event.target.value * 1
      //如果用户输入进来的非法数字,出现NAN或者小于1
      if(isNaN(value)||value<1){
        this.skuNum = 1
      }else{
        // 正常大于1 【大于1整数 不能出现小数 用parseInt取整】
    ...
    async addShopCart(){
      ...
      // try catch try 表示成功了做什么 catch表示 失败了做什么
      try {
        ...
        // 本地存储|会话存储，一般存储的是字符串 
        // JSON.stringify 把对象转换成字符串 可以用JSON.parse 把字符串转换成对象接收
