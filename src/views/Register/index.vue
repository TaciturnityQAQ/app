<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>登录密码:</label>
        <input placeholder="请输入你的登录密码" type="password" v-model="password" name="password" v-validate="{ required: true, regex: /^[0-9A-Za-z]\d{8,20}$/ }" :class="{ invalid: errors.has('password') }"/>
        <span class="error-msg">{{ errors.first("password") }}</span>
      </div>
      <div class="content">
        <label>确认密码:</label>
        <input placeholder="请输入再次输入确认密码" type="password" v-model="password1" name="password1" v-validate="{ required: true,is:password }" :class="{ invalid: errors.has('password1') }"/>
