        ...
        >
          <li class="cart-list-con1">
            <input
              ...
              @change="upDateCheckCart(cart.skuId,$event)"
            />
          </li>
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteCartById(cart.skuId)"
              >删除</a
            >
            <br />
            <a href="#none">移到收藏</a>
...
export default {
  ...
  methods: {
    ...
    // 修改某一个产品的个数 防止用户点击过快 使用节流 throttle
    ...
    // 切换某一个商品选中状态 
    async upDateCheckCart(skuId,event){
      // 带给服务器的参数isChecked，不是布尔值,应该是 0|1
      try {
        // 如果修改数据成功,再次获取服务器数据〔购物车)
        let isChecked = event.target.checked ? '1':'0'
        await this.$store.dispatch('upDateCheckCart',{skuId,isChecked})
        this.getData()
      } catch (error) {
        alert(error.message)
      }
  ...
  computed: {
    ...
    isAllCheck() {
      // every遍历数组的原理，只要全部元素isChecked属性都为1 ===> 返回真 true
      // 只要有一个不是1 ======> 返回假false
